"""
Evaluate the histogram gradient boosting tabular baseline.
"""
...
from src.config import PROCESSED_DIR, hist_gradient_boosting_dirs
...
def main():
    print("=" * 60)
    print("Evaluation - HistGradientBoosting baseline")
    print("=" * 60)
    ...
    with open(os.path.join(METRICS_DIR, "hist_gradient_boosting_metrics.json"), "w") as f:
        json.dump(metrics, f, indent=4)
    ...
    plt.title("Normalized Confusion Matrix - HistGradientBoosting")
    plt.xlabel("Predicted Class")
    plt.ylabel("True Class")
    plt.tight_layout()
    ...
    plt.title("ROC Curves - HistGradientBoosting")
    ...
    for i, cls_name in enumerate(class_names):
        ...
        plt.plot(
            ...
            color=COLORS[i], lw=2, label=f"{cls_name} (AP={ap:.3f})"
        )
    plt.xlabel("Recall")
    plt.ylabel("Precision")
    plt.title("Precision-Recall Curves - HistGradientBoosting")
    ...
    np.save(os.path.join(METRICS_DIR, "y_pred_hist_gradient_boosting.npy"), y_pred)
    np.save(
        os.path.join(METRICS_DIR, "y_pred_proba_hist_gradient_boosting.npy"),
        y_pred_proba,
    )

    print(f"Saved HistGradientBoosting results in {DIRS['root']}")
