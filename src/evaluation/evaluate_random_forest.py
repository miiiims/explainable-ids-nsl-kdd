...
This script charge le modèle Random Forest entraîné sur les features normalisées
de NSL-KDD et génère les métriques & visualisations standard.
"""
...
from src.config import PROCESSED_DIR, random_forest_dirs
...
def main():
    print("=" * 60)
    print("Evaluation - Random Forest baseline")
    print("=" * 60)
    ...
    np.save(os.path.join(METRICS_DIR, "y_pred_random_forest.npy"), y_pred)
    np.save(os.path.join(METRICS_DIR, "y_pred_proba_random_forest.npy"), y_pred_proba)

    print(f"Saved Random Forest results in {DIRS['root']}")
