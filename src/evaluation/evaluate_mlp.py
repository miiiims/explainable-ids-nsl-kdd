"""
Evaluate Experiment 2: lightweight tabular MLP.
"""
...
from src.config import PROCESSED_DIR, mlp_dirs
...
def main():
    ...
    with open(os.path.join(METRICS_DIR, "mlp_metrics.json"), "w") as f:
        json.dump(metrics, f, indent=4)
    ...
    plt.title("Precision-Recall Curves - MLP")
    ...
    np.save(os.path.join(METRICS_DIR, "y_pred_mlp.npy"), y_pred)
    np.save(os.path.join(METRICS_DIR, "y_pred_proba_mlp.npy"), y_pred_proba)

    print(f"\nSaved MLP results in {DIRS['root']}")
